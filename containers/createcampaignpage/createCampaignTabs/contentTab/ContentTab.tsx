...
import { faCaretDown, faCaretLeft, faCirclePlus, faCircleXmark, faSquarePen, faSquareXmark } from "@fortawesome/free-solid-svg-icons"
...
export default function ContentTab() {
  ...
  const { updateGrandCmp, newCampaignAddr, setActiveTab, grandCmp, setNewCampaignAddr } = useContext(CampaignCreatorContext)!
  ...
  async function uploadImg (blobInfo:any, progress:any){
    ...
    const imgData = {
      ...
      key: `manger/storyImgUploads/${truncateStr(account, 10)}/MNG_${timestamp}`,
      data: blobInfo.blob()
    }
    try {
    ...
    <>
      {
        !newCampaignAddr
          ? <div className="ct-denial">
            <p>{"You need an existing campaign smart contract to create content"}</p>
          ...
          "be sure to include frequently asked questions (FAQ), to help your description"}
            ...
            <div className="ct-container">
              <div className="ct-submit">
                <button className="ct-submit-btn" onClick={()=>{handleContentSave()}} disabled={scbtn}>{"Save"}</button>
              </div>
              {/* campaign story */}
              ...
              "A good pitch is compelling, informative, and easy to digest. Learn more."}
                ...
                <Editor 
                  plugins={"image link preview table paste wordcount visualblocks searchreplace"} 
                  toolbar={"undo redo | formatselect | bold italic backcolor | fontsize blocks | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link image preview"} 
                  init={{ width:"100%", height:700, menubar:false, images_upload_handler: uploadImg }}
                  value={story}
                  onEditorChange={(newText)=>{setStory(newText)}}
                />
              </div>
              ...
              <div className="ct-card">
                ...
                <Editor 
                  plugins={"link preview table paste wordcount visualblocks searchreplace"} 
                  toolbar={"undo redo | formatselect | bold italic backcolor | fontsize blocks | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link removeformat preview"} 
                  init={{ width:"100%", height:300, menubar:false, images_upload_handler: uploadImg }}
                  value={risks}
                  onEditorChange={(newText)=>{setRisks(newText)}}
                />
              </div>
            ...
            "questions about crowdfunding and how Manger works"}
                  ...
                  return (
                    <div key={index} className="ct-faqbox">
                      ...
                      <div className="ct-faqbox-options">
                        <FontAwesomeIcon icon={faSquarePen} className="ct-faqbox-edit-icon"
                          onClick={()=>{editFaq(faq, index)}}
                        />
                        <FontAwesomeIcon icon={faSquareXmark} className="ct-faqbox-x-icon"
                          onClick={()=>{faqs.splice(index,1); setFaqs(prev => [...faqs])}}
                        />
                      </div>
                    </div>
                  )
                ...
                <div className="ct-faq-maker">
                  <p className="ct-faq-maker-heading"
                    style={showFaqInput ? { "display":"none" } : {}}
                  >{"Add a question"}</p>
                  <div className="ct-faq-cta" onClick={()=>{setShowFaqInput(true)}}
                    style={showFaqInput ? { "display":"none" } : {}}
                  >
                    <FontAwesomeIcon icon={faCirclePlus} className="ct-faq-add-icon"/>
                  </div>

                  <form className="ct-faq-qa" 
                    style={showFaqInput ? { "display" : "flex" } : { "display" : "none" }} 
                    onSubmit={(e)=>{e.preventDefault(); updateFaqs()}}
                  >
                    <div className="ct-faq-inpt">
                      <p>{"Question"}</p>
                      <input type="text" name="faq-q" id="faq-q" className="ct-faq-q" 
                        onChange={(e)=>{setCurrFaqQ(e.target.value)}} value={currFaqQ}
                        required
                      />
                    </div>
                    <div className="ct-faq-inpt">
                    ...
                    <div className="ct-faq-add">
                      ...
                      <button className="ct-faq-cancel-btn" type="button" onClick={()=>{setShowFaqInput(false)}}>
                        {"Cancel"}
                      </button>
                    </div>
                  </form>
              ...
              <div className="ct-submit">
                <button className="ct-submit-btn" onClick={()=>{handleContentSubmit()}}>{"Save & Continue"}</button>
              </div>
            </div>
          </div>
      }
