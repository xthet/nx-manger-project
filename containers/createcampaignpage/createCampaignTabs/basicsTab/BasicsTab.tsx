...
export default function BasicsTab() {
  const { activeTab, setActiveTab, setNewCampaignAddr, grandCmp, updateGrandCmp, newCampaignAddr } = useContext(CampaignCreatorContext)!
  ...
  async function uploadImg(e:any){
    ...
    const imgData = {
      ...
      key: `manger/cmpImgUploads/${truncateStr(account, 10)}/MNG_${cmpTitle}_${timestamp}`,
  ...
  const handleBasicsSubmit = onetime(async ()=>{
    ...
    if(currCat && tagArr.length > 0 && imgURLToBe){
      ...
      try {
        ...
        // setSubmitted(true)
        setTimeout(()=>{setActiveTab("Rewards")},1000)
      } catch (error) {
        dispatch({
          type: "ADD_NOTI",
          payload:{
            ...
            type: "FAILURE",
            title: "Error",
            message: "Campaign smart contract creation failed \nMake sure you entered all the required info"
          }
        })
        console.log(error)
        setShowTBX(false)
  ...
  useEffect(()=>{
    ...
    if(basicRec){
      ...
      setDur((parseInt(ethers.utils.formatUnits(recObj.duration, 0)) / 86400).toString())
  ...
  return (
    <div className="bt-tab fl-tl fl-c" id="basics">
      ...
      <form className="bt-form-container fl-cl fl-c" 
        onSubmit={(e)=>{e.preventDefault(); !durNotice && handleBasicsSubmit()}}
      >
        {/* campaign title */}
        ...
        <div className="bt-card fl-tl fl-c ">
          ...
          <div className="bt-card-input fl-bl fl-c">
            <textarea name="bt-tagline" 
              id="bt-tagline" cols={91} rows={2} 
              className="bt-card-txt-ta" maxLength={100} 
              onChange={(e)=>{setCmpTagLine(e.target.value)}} value={cmpTagLine}
              required
            ></textarea>
        ...
        <div className="bt-card fl-tl fl-c" id="bt-ipfs-upld">
          ...
          <div className="bt-card-img-input fl-cl fl-c">
            <input type="file" id="bt-card-img" hidden onChange={(e)=>{uploadImg(e); setImgState("loading")}} required/>
        ...
        <div className="bt-card fl-tl fl-c" id="bt-cats">
          ...
          <div className="bt-card-select fl-cl fl-sb">
            <div className="bt-card-categories fl-cl">
              ...
              <CatTable showTable={showCatTable} setCurrCat={setCurrCat} setShowTable={setShowCatTable}/>
            </div>
        ...
        <div className="bt-card fl-tl fl-c ">
          ...
          <div className="bt-card-select fl-cl hide">
                  ...
                  return (
                    <span className="bt-card-tag fl-cc" key={index}>
                      ...
                      <FontAwesomeIcon icon={faXmark} className="bt-xmark-icon" 
                        onClick={()=>{tagArr.splice(index,1); setTagArr(prev=>[...tagArr])}}
                      />
                    </span>
                  )
                })
            ...
            <input 
              ref={tagInput}
              type="text" className="bt-tag-input" 
              maxLength={20} placeholder="Enter a few tags for your c..."
              onChange={(e)=>{setCurrTag(e.target.value)}}
              onKeyDown={(e)=>{checkKey(e)}}
            />
          </div>
          <p className="bt-card-char-count" 
            style={tagNotice ? { "display": "block", "color": "red" } : { "display": "none" }}
          >{"You can only enter up to three keywords"}</p>
        </div>
        ...
        <div className="bt-card fl-tl fl-c ">
          ...
          <p className="bt-card-subtitle">{"How much do you aim to raise from this campaign?"}</p>
          <div className="bt-card-fund fl-cl">
            {"ETH"}
            <input type="number" className="bt-card-fund-input" 
        ...
        <div className="bt-card fl-tl fl-c ">
          ...
          <div className="bt-card-input fl-tl fl-c">
            <input type="number" className="bt-card-num-input" 
              onChange={(e)=>{parseInt(e.target.value) > 60 ? setDurNotice(true) : setDurNotice(false); setDur((e.target.value))}} 
              value={dur} min={1} max={60} required
            />
          </div>
          <p className="bt-card-char-count" 
            style={durNotice ? { "display": "block", "color": "red" } : { "display": "none" }}
          >{"You may only set up to 60 days"}</p>
        </div>

        <div className="bt-submit fl-cr">
          <button type="submit" className="bt-submit-btn">{"Save & Continue"}</button>
        </div>
      </form>
    </div>
  )
