...
import { basicCmpObj, conn, counOpt, teamCmpObj } from "@/types"
import getCountrySelect from "@/utils/getCountrySelect"
import { truncateStr } from "@/utils/truncateStr"
import fleek from "@fleekhq/fleek-storage-js"
import { faCaretLeft, faCircleCheck, faCirclePlus, faCircleXmark } from "@fortawesome/free-solid-svg-icons"
...
export default function TeamTab() {
  ...
  const { updateGrandCmp, newCampaignAddr, setActiveTab, grandCmp, setNewCampaignAddr, setGrandURI, activeTab } = useContext(CampaignCreatorContext)!
  ...
  async function handleTeamSave(){
    ...
    localStorage.setItem("teamObj", JSON.stringify(teamObj))
  }

  const handleTeamSubmit = onetime(async (bypass = true)=>{
    if(cmpEmail && location && cmpBio && Number(ilinkBal) && Number(ilinkBal) > 2){
      const teamObj:teamCmpObj = {
        creators: modArr(),
        email: cmpEmail,
        website: cmpWeb,
      ...
      setShowTBX(false); setTlIndex(0)   
    }
  })

  const uploadGrandCmp = onetime(async () => {
    setScbtn(true)
    setTimeout(()=>{setScbtn(false)},15000)
    setShowTBX(true)
    const timestamp = (new Date).getTime()
    const grandCmpData = {
      apiKey: process.env.NEXT_PUBLIC_FLEEK_STORAGE_API_KEY!,
      apiSecret: process.env.NEXT_PUBLIC_FLEEK_STORAGE_API_SEC!,
      key: `manger/grandCmpUplds/${truncateStr(account, 10)}/MNG_${grandCmp.title}_${timestamp}`,
      data: JSON.stringify(grandCmp)
    }
    const cmpContract = new ethers.Contract(newCampaignAddr, Campaign.abi, signer)
    ...
    try {
      ...
      const updateURITx = await cmpContract.updateCampaignURI(`ipfs://${response.hashV0}`)
      ...
      if(hasKey(TimeboxContracts, chainId)){
        ...
        // campaign published
        const publishTxR = await publishTx.wait(1)
      ...
      dispatch({
        ...
        payload:{
          ...
          message: "Sorry ;) we could not create this campaign"
        }
      })
      console.log(error)   
      setShowTBX(false); setTlIndex(0)   
  ...
  useEffect(()=>{
    async function getLinkBal(){
      if(hasKey(TimeboxContracts, chainId)){
        setIlinkLdng(true)
        const linkCt = new ethers.Contract(TimeboxContracts[chainId].linkToken, LinkToken.abi, signer)
        try {
    ...
    const inr = setInterval(()=>{getLinkBal().catch(e=>console.log(e))}, 6000)
  ...
  useEffect(()=>{
    ...
    const teamRec = localStorage.getItem("teamObj")
    // console.log(teamRec)
    if(teamRec){
      const recObj:teamCmpObj = JSON.parse(teamRec)
      setAddArr(recObj.creators)
      setCmpEmail(recObj.email)
      setCmpWeb(recObj.website)
      setCmpBio(recObj.bio)
      setLocation(recObj.location)
      setSelectedOption({ value:recObj.location, label:recObj.location })
      setCmpTwit(recObj.twitter == "_NIL" ? "" : recObj.twitter)
    ...
    <>
      {
        !newCampaignAddr
          ? <div className="ct-denial">
            <p>{"You need an existing campaign smart contract to add team info"}</p>
          ...
          "your campaign page as members of your team."}
            ...
            <div className="ct-container">
              <div className="ct-submit">
                <button className="ct-submit-btn" onClick={()=>{handleTeamSave()}}>{"Save"}</button>
              </div>
              <div className="ct-card">
                ...
                <p className="ct-card-subtitle">
                  {"Add members of your campaign here:"}
                </p>
                ...
                <div className="tt-members-inpt-grp">
                  ...
                  <div className="tt-members-inpt-list-grp">
                    <div className="tt-address-inpt-grp">
                      <input type="text" name="wallet" className="tt-address-inpt"
                          ...
                          modArr().map((addr, index)=>{
                            return (
                              <div className="tt-member-grp" key={index}>
                                <li className="tt-member">{truncateStr(addr, 14)}</li>
                                <FontAwesomeIcon icon={faCircleXmark} className="tt-del-member-icon"
                                  onClick={()=>{addArr.splice(index,1); setAddArr(prev=>([...addArr]))}}
                                />
                              </div>
                            )
                          })
              ...
              <form className="rt-card-form" onSubmit={(e)=>{e.preventDefault()}}>
                <div className="ct-card-heading">
                ...
                <div className="rt-form-inpt-grp">
                  <p className="rt-form-inpt-label">{"Team Email:"}</p>
                  <div className="rt-form-inpt-sm-grp">
                    <small>{"What email should backers use to reach you?"}</small>
                    <input type="email" name="t-email" className="rt-form-inpt"
                      onChange={(e)=>{setCmpEmail(e.target.value)}} value={cmpEmail}
                      required
                    />
                ...
                <div className="rt-form-inpt-grp">
                  <p className="rt-form-inpt-label">{"Team Website:"}</p>
                  <div className="rt-form-inpt-sm-grp">
                    <small>{"Does your campaign have a website?"}</small>
                    <input type="text" name="t-web" className="rt-form-inpt"
                      onChange={(e)=>{setCmpWeb(e.target.value)}} value={cmpWeb}
                    />
                  </div>
                </div>
                ...
                <div className="rt-form-inpt-grp">
                  ...
                  <div className="rt-form-inpt-sm-grp">
                    <small>{"Does your campaign have an official twitter page?"}</small>
                    <input type="text" name="t-twit" className="rt-form-inpt"
                      placeholder="username" value={cmpTwit}
                      onChange={(e)=>{setCmpTwit(e.target.value)}}
                    />
                    <small>{cmpTwit && "https://www.twitter.com/" + cmpTwit}</small>
                  </div>
                </div>
                ...
                <div className="rt-form-inpt-grp">
                  ...
                  <div className="rt-form-inpt-sm-grp">
                    ...
                    <Select options={counOptionsArr} 
                      ...
                      value={selectedOption} className="tt-form-select"
                ...
                <div className="rt-form-inpt-grp">
                  ...
                  <div className="rt-form-inpt-sm-grp">
                    ...
                    <textarea name="t-bio" className="rt-form-inpt" cols={91} rows={5}
                      onChange={(e)=>{setCmpBio(e.target.value)}} value={cmpBio}
                      required
                    />
                    <small style={{ "color":"orange" }}>{ "required" }</small>
                  </div>
              ...
              <div className="ct-card">
                ...
                <p className="tt-card-subtitle">
                  ...
                  {chainId == "1" ? <i className="tt-get-link">{"Swap some ETH for LINK"}</i> : <a href="https://faucets.chain.link/" target="_blank" rel="noopener noreferrer" className="tt-get-link">{"get some LINK here"}</a>}
                </p>
                <div className="tt-link-bal-wrp">
                  ...
                  <div className="tt-link-bal-cont">
                    ...
                    <img src="/assets/chainlink-link-logo.svg" alt="--" className="tt-link-logo"/>
                    <p className="tt-link-bal">{ ilinkBal + " LINK"}</p>
                    {ilinkLdng ? <ReactLoading type="bubbles" color="#827B93" width={22} height={22}/> : (Number(ilinkBal) > 2) ? <FontAwesomeIcon icon={faCircleCheck} className="tt-link-check"/> : <Link href={chainId == "1" ? "#" : "https://faucets.chain.link/"} target={chainId == "1" ? "_self" : "_blank"} ><button className="tt-get-link-btn">{"GET LINK"}</button></Link>}
                  </div>
                </div>
              </div>

              <div className="tt-cta">
                <button className="tt-cta-btn" onClick={(e)=>{addiRef.current!.click(); handleTeamSubmit(false)}}>
                  {"Publish Campaign"}
                </button>
              </div>
            </div>
