...
export default function EBasicsTab() {
  const { activeTab, setActiveTab, updateGrandCmp, currAddress } = useContext(CampaignEditorContext)!
  ...
  return (
    <div className="bt-tab fl-tl fl-c" id="basics">
      ...
      {loading || !campaignDetails.campaignURI 
        ...
        >
          ...
          <div className="bt-card fl-tl fl-c ">
            ...
            <div className="bt-card-img-input fl-cl fl-c">
              <input type="file" id="bt-card-img" hidden onChange={(e)=>{uploadImg(e); setImgState("loading")}} disabled/>
              <label htmlFor="bt-card-img" className="bt-img-label bt-inpt-disabled">
                <div className="bt-img-container fl-cc">
                  {imgState == "loading" 
                    ? <ReactLoading type="bubbles" color="#827B93"/> 
                    : imgState == "finished" 
                      ? imgURLToBe && <img src={imgURLToBe.replace("ipfs://", "https://ipfs.io/ipfs/")} alt="img-uploaded" /> 
          ...
          <div className="bt-card fl-tl fl-c" id="id-updt-dur">
            ...
            <div className="bt-card-input fl-cl">
              <input type="number" className="bt-card-num-input" 
                onChange={(e)=>{checkNewDur(parseInt(e.target.value)); setAddableDur((e.target.value)) }} 
                value={addableDur} min={1} max={60}
              />

              <button className="bt-dur-max" onClick={()=>{maxAddableDur()}}>
                {"MAX"}
              </button>
            </div>
            <p className="bt-card-char-count" 
          ...
          <div className="bt-submit fl-cr">
            <button type="submit" className="bt-submit-btn" onClick={()=>{!durNotice && handleDurUpdate()}}>{"Save & Continue"}</button>
          </div>
        </form>
      }
