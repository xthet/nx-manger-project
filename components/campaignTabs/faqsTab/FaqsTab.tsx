                    ...
                    <div className="ct-faq-box-qa">
                      <p className="ct-faqbox-q">{faq.question}
                        {!(showFAns == index) ? <FontAwesomeIcon icon={faCaretDown} className="ct-faqbox-show-icon" onClick={()=>{setShowFAns(index)}}/>
                          : <FontAwesomeIcon icon={faCaretUp} className="ct-faqbox-show-icon" onClick={()=>{setShowFAns(-1)}}/>}
                      </p>
                      {(showFAns == index) && <p className="ct-faqbox-a">{faq.answer}</p>}
                    </div>
                  </div>
                )}) 
                : <>
                  <p>{`Looks like there aren't any frequently asked questions yet. ${isAuth ? "Ask the project creator directly." : ""}`}</p>
                  {isAuth && <Link href={`mailto:${cdata.email}`}><button className="st-faq-cta">{"Ask a question"}</button></Link>}
                </>
            }
          </div>
      }
